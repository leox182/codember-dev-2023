const passwords = `8-10 r: ozrcdfnug
9-10 q: hvsazxrigf
1-9 j: bbqonxjt
...
function passwordsToArray(passwords) {
  return passwords.split("\n");
}

function getInvalidPasswords(passwords) {
  const passwordList = passwordsToArray(passwords);
  const wrongPasswords = [];

  passwordList.forEach((element) => {
    const [minNumber, maxNumber, charToCheck, _, password] = element.split(/[- :]/);
    let count = 0;
    ...
    if (count < minNumber || count > maxNumber) wrongPasswords.push(password);
  });

  return wrongPasswords;
}

console.log(getInvalidPasswords(passwords)[12]);
